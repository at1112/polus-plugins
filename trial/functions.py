...
try: 
    ...
    if __name__=="__main__":
        ...
        for f in images_files:
            ...
            a = br.read_image(X=(0,2048),Y=(0,2048))
            #a = br.read_image()        
            image = np.squeeze(a)
            ...
            iterable = ij.op().convert().int64(arg_in)
            print(type(iterable))
    ...
    '''
        ...
        bw.num_x(output_np.shape[1])
        bw.num_y(output_np.shape[0])
        bw.num_z(output_np.shape[2])
        bw.num_c(output_np.shape[3])
        bw.num_t(output_np.shape[4])
